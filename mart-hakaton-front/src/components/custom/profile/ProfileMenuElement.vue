...
<template>
  <Collapsible class="flex flex-col w-full h-fit" v-model:open="isOpen">
    <CollapsibleTrigger class="w-full"
      ><Button variant="ghost" class="flex justify-between w-full" @click="makeSelected">
        <div class="flex justify-start gap-3">
          <CornerDownRight v-if="isSelect" />
          <slot name="icon"></slot>
          <slot></slot>
        </div>
        <ChevronDown v-show="isSubmenu && isOpen" />
        <ChevronRight v-show="isSubmenu && !isOpen" /> </Button
    ></CollapsibleTrigger>
